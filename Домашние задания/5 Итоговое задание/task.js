...
class Player {
  ...
  getDistance(enemy) {
    return Math.abs(this.position - enemy.position);
  }
  ...
  checkWeapon() {
    ...
    const currentWeaponIndex = this.weaponPriority.findIndex(
      WeaponClass => this.weapon.constructor === WeaponClass
    );

    const NextWeapon = this.weaponPriority[currentWeaponIndex + 1];
  ...
  chooseEnemy(players) {
    return players
      ...
      .reduce((enemyWithMinLife, player) => {
        if (!enemyWithMinLife || player.life < enemyWithMinLife.life) {
          return player;
        }

        return enemyWithMinLife;
      }, null);
  }
...
class Dwarf extends Warrior {
  ...
  takeDamage(damage) {
    ...
    if (this.enemyHits % 6 === 0 && this.getLuck() > 0.5) {
      super.takeDamage(damage / 2);
      return;
    }
